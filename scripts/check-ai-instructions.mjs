...
// - `required:` followed by `  - fieldname` items (2-space indent)
// - `properties:` with 2-space-indented keys, 4-space-indented `type:` values
// Reformatting (tabs, different indent, flow syntax) will cause empty results.
...
async function main() {
  ...
  if (!schemaFields.rule || !schemaFields.skill || !schemaFields.agent) {
    warn(
      '[check] some schema definitions could not be parsed — using fallback field lists',
    );
  }
  ...
  if (!schemaTypes.rule || !schemaTypes.skill || !schemaTypes.agent) {
    warn(
      '[check] some schema type maps could not be parsed — using fallback type maps',
    );
  }
