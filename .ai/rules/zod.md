---
id: zod
intent: 'Zod schema authoring — strictObject, safeExtend, meta(), and composition layers'
scope:
  - 'src/**/*.schema.ts'
  - 'src/**/*.schemas.ts'
requirements:
  - 'Use `z.strictObject()` (not `z.object().strict()`)'
  - 'Compose via `.safeExtend()` (Base -> NFT -> type-specific)'
  - 'Add `.meta()` with title/description/example(s) to every field'
  - 'PascalCase schema names (`[Entity]Schema`) and types (`[Entity]`)'
  - 'Minimize `.refine()` — only for cross-field or domain-specific validation'
  - 'Extract schemas when reused by 2+ consumers or when they have 3+ fields'
anti_patterns:
  - 'Using `z.object().strict()` instead of `z.strictObject()`'
  - 'Missing `.meta()` on fields'
  - 'Using `.refine()` when built-in validators suffice'
  - 'Inline schemas that should be extracted'
  - '`.merge()` instead of `.safeExtend()`'
