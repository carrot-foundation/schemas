...
intent: 'pnpm only, exact versions, Node >= 22, and minimal dependencies'
scope:
  - 'package.json'
  - 'pnpm-lock.yaml'
requirements:
  - 'pnpm only (enforced via preinstall script)'
  - 'Exact versions (`pnpm add -E`)'
  - 'Node >= 22'
  - 'Evaluate necessity before adding dependencies'
  - 'Keep production dependencies minimal (currently only zod + canonicalize)'
  - 'Document why new dependencies are added'
anti_patterns:
  - 'Using npm/yarn'
  - 'Version ranges (^, ~)'
  - 'Adding dependencies without justification'
  - 'Bundling dev dependencies as production'
