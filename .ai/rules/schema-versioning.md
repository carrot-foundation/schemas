...
intent: 'Schema version injection, $id format, and SCHEMA_VERSION environment variable'
scope:
  - 'src/shared/schema-version.ts'
  - 'scripts/**'
requirements:
  - 'Version sourced from SCHEMA_VERSION env var (falls back to package.json version)'
  - '$id format uses git tags URL'
  - 'Unified dev/prod versioning approach'
  - 'Version embedded during build via tsup env'
  - 'Verify versions with `pnpm verify-schema-versions`'
anti_patterns:
  - 'Hardcoding version strings'
  - 'Different versioning logic for dev vs prod'
  - 'Bypassing SCHEMA_VERSION injection'
  - 'Manually updating $id URLs'
---
